...
Mission3::Mission3(int _width, int _height)
{

	_wnd = new RenderWindow(VideoMode(_width, _height), "Mission 3");
	_fps = 60;
	_frametime = 1.f / _fps;
	...
	_M1Tx = new Texture;
	_M1Sp = new Sprite;
	_M1Tx->loadFromFile("Asset/Mission_3.png");
	_M1Sp->setTexture(*_M1Tx);
	_M1Sp->setScale(Vector2f(0.055f, 0.095f));
...
void Mission3::CreateAndShootRagdoll(float angle, float force)
{
...
void Mission3::UpdateCannonPosition()
{
...
void Mission3::DrawGame()
{

	_wnd->draw(*_M1Sp);
}

void Mission3::GamePause()
{
